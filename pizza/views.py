...
def order(request):
	...
	if request.method == 'POST':
		filled_form = PizzaForm(request.POST) # , request.FILES untuk upload file
...
def pizzas(request):
		...
		if request.method == 'POST':
			...
			if (filled_formset.is_valid()):
				for form in filled_formset:
					...
					created_pizza1 = form.save()
					# created_pizza_pk1 = created_pizza1.id
					note = 'Thanks for ordering! Your %s pizzas is on its way!' %(number_of_pizzas)
					form = PizzaFormSet()
			else:
	...
	# 	raise KeyError('Order was not created, please try again.')
	# finally:
	# 	order(request)
